...
- IDENTITY PRESERVATION: When calling tools (like `calculate_adjusted_value`), you MUST include the `entity_identifier` (e.g., brand/tenant name) and a `metrics` dictionary containing raw values. This ensures the output can be accurately mapped back to the source.
